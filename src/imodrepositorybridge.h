...
Q_SIGNALS:
   ...
   * following keys are valid as of this writing:
   * 'allow_view', 'ip', 'one_week_ratings', 'date', 'pm_notify', 'OLD_mid',
   * 'OLD_u_downloads', 'game_id', 'OLD_perm_use', 'mod_page_uri', 'allow_topics',
   * 'has_hot_image', 'id', 'two_weeks_ratings', 'description', 'lastupdate',
   * 'perm_convert', 'author', 'OLD_image', 'translation_of', 'OLD_mname', 'version',
   * 'allow_rating', 'perm_useinstructions', 'featured_count', 'donate', 'type',
   * 'perm_credits', 'hidden_reason', 'OLD_views', 'perm_upload', 'has_back_image',
   * 'adult', 'allow_images', 'OLD_endorsements', 'OLD_size', 'name', 'commenting',
   * 'moderate', 'language', 'perm_others', 'lastcomment', 'OLD_readme', 'summary',
   * 'perm_modify', 'OLD_downloads', 'lock_comments', 'suggested_category',
   * 'allow_tagging', 'published', 'perm_notes', 'category_id', 'thread_id', 'perm_use',
   * 'wizard_steps'
   *
   ...
   * @note valid keys might change as the repository page is updated. For nexus, the
   * following keys are valid as of this writing:
   * 'count', 'requirements_alert', 'u_count', 'description', 'uri', 'size', 'owner_id',
   * 'primary', 'manager', 'version', 'date', 'game_id', 'mod_id', 'category_id', 'id',
   * 'name'
  ...
  /**
   * @brief sent when the list of download urls for a file is returned by the
   * repository
   *
   * @param modID id of the mod for which the request was made
   ...
   *
   * @note this function is not exposed to python. please
   * use IDownloadManager::startDownloadNexusFile to download a file, this lets
   * the download manager figure out the best server according to user preference and
   * stuff
   * @note this interface is going to be changed at some point to replace
   * resultData with a less "dynamic" data structure
   */
  ...
  /**
   * @brief sent when the endorsement state of a mod was changed (only sent as a
  ...
  /**
   * @brief sent when the tracking state of a mod was changed (only sent as a
   * result of our request)
   *
   * @param modID id of the mod for which the request was made
   * @param userData the data that was included in the request
