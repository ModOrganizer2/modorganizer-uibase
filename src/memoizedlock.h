...
/**
 * @brief Class that can be used to perform thread-safe memoization.
 *
 * Each instance hold a flag indicating if the current value is up-to-date or not. This
 * flag can be reset using `invalidate()`. When the value is queried, the flag is
 * checked, and if it is not up-to-date, the given callback is used to compute the
 * value.
 *
