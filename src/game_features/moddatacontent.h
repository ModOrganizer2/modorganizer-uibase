...
/**
 * The ModDataContent feature is used (when available) to indicate to users the
 * content of mods in the "Content" column.
 *
 * The feature exposes a list of  possible content types, each associated with an ID,
 * a name * and an icon. The icon is the path to either:
 *   - A Qt resource or;
 *   - A file on the disk.
 *
...
public:
  struct Content
  {
     ...
     * @param icon Path to the icon for this content. Can be either a path to an image
     * on the disk, or to a resource. Can be an empty string if filterOnly is true.
