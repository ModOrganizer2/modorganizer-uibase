...
public:
  ...
  /**
   * @brief Check that the given filetree represent a valid mod layout, or can be
   * easily fixed.
   *
   * This method is mainly used during installation (to find which installer should be
   * used or to recurse into multi-level archives),  or to quickly indicates to a user
   * if a mod looks valid.
   *
   ...
   * should be avoided).
   *
   * If the tree can be fixed by the `fix()` method, this method should return
   * `FIXABLE`. `FIXABLE` should only be returned when it is guaranteed that `fix()` can
   * fix the tree.
   *
   ...
   */
  virtual CheckReturn
  dataLooksValid(std::shared_ptr<const MOBase::IFileTree> fileTree) const = 0;

  /**
   * @brief Try to fix the given tree.
   *
   * This method is used during installation to try to fix invalid archives and will
   * only be called if dataLooksValid returned `FIXABLE`.
   *
