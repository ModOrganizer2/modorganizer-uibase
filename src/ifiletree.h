 ...
 * inherited by IFileTree so that operations on entry are the same for a file or     a
 * directory.
 *
...
public:  // Methods
   ...
   *
   * @return the path from this entry to the root, including the name of this entry.
   */
   ...
   * @param sep The type of separator to use to create the path.
   *
   * @return the path from this entry up to the given tree, including the name of this
   * entry, or QString() if the given tree is not a parent of this one.
   */
   ...
   *
   * @return the parent tree containing this entry, or a null pointer if this entry is
   * the root or the parent tree is unreachable.
   */
...
public:  // Access methods:
   ...
   * @param sep The type of separator to use to create the path.
   *
   * @return the path from this tree to the given entry, including the name of the
   * entry, or QString() if the given entry is not in this tree.
   */
...
public:  // Mutable operations:
   ...
   * @param replaceIfExists If true and an entry already exists at the given path, it
   * will be replaced by a new entry. This will replace both files and directories.
   *
   * @return the entry corresponding to the create file, or a null pointer if the file
   * was not created.
   */
   ...
   * directory already exists but will fail if the given path contains "." or "..". This
   * method invalidates iterators to this tree and all the subtrees present in the given
   * path.
   *
   * @param path Path to the directory.
   *
   * @return the entry corresponding to the created directory, or a null pointer if the
   * directory was not created.
   */
  virtual std::shared_ptr<IFileTree> addDirectory(QString path);

  /**
   * @brief Insert the given entry in this tree, removing it from its previous parent.
   *
   * The entry must not be this tree or a parent entry of this tree.
   *
   * - If the insert policy if FAIL_IF_EXISTS, the call will fail if an entry with the
  ...
  /**
   * @brief Merge the given tree with this tree, i.e., insert all entries of the given
   * tree into this tree.
   *
   ...
   * @param overwrites If not null, can be used to create a mapping from overriden file
   * to new files.
   *
   * @return the number of overwritten entries, or MERGE_FAILED if the merge failed
   * (e.g. because the source is a parent of this tree).
   */
   ...
   *
   * @param entry Entry to insert.
   * @param path The path to move the entry to. If the path ends with / or \, the entry
   * will be inserted in the corresponding directory instead of replacing it. If the
   ...
   * @param path The path to copy the entry to. If the path ends with / or \, the entry
   * will be inserted in the corresponding directory instead of replacing it.
   * @param insertPolicy Policy to use on conflict.
   *
   ...
   *
   * @param entry Entry to delete. The entry must belongs to this tree (and not to a
   * subtree).
   *
   * @return an iterator following the removed entry (might be the end iterator if the
   * entry was not found or was the last).
   */
   ...
   *
   * @return a pair containing an iterator following the removed entry (might be the end
   * iterator if the entry was not found or was the last) and the removed entry (or a
...
public:  // Inherited methods:
   ...
   * if this entry corresponds to a file.
   *
   * @return the tree corresponding to this entry, or a null pointer if isDir() is
   * false.
   */
