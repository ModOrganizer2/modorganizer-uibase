...
/**
 * @brief Interface to class that provides information about the running session of Mod
 * Organizer to be used by plugins.
 *
 * When MO requires plugins but does not have a valid instance loaded (such as on first
 * start in the instance creation dialog), init() will not be called at all, except for
 * proxy plugins.
 *
...
public:
   ...
   *
   * @param executable either the name of an executable configured in MO, or a path to
   * an executable; if relative, it is resolved against the game directory
   * @param args arguments to pass to the executable; if this is empty and `executable`
   * refers to a configured executable, its arguments are used
   * @param cwd working directory for the executable; if this is empty, it is set to
   * either the cwd set by the user in the configured executable (if any) or the
   * directory of the executable
   * @param profile name of the profile to use; defaults to the active profile
   * @param forcedCustomOverwrite the name of the mod to set as the custom overwrite
   * directory, regardless of what the profile has configured
   * @param ignoreCustomOverwrite if `executable` is the name of a configured
   * executable, ignores the executable's custom overwrite
   *
   * @return a handle to the process that was started or INVALID_HANDLE_VALUE if the
   ...
   *
   * @note this will always show the lock overlay, regardless of whether the user has
   * disabled locking in the setting, so use this with care; note that the lock overlay
   * will always allow the user to unlock, in which case this will return false
   */
