...
concept BaseGameFeature = requires(T) {
  std::is_same_v<T, BSAInvalidation> || std::is_same_v<T, DataArchives> ||
      std::is_same_v<T, GamePlugins> || std::is_same_v<T, LocalSavegames> ||
      std::is_same_v<T, ModDataChecker> || std::is_same_v<T, ModDataContent> ||
      std::is_same_v<T, SaveGameInfo> || std::is_same_v<T, ScriptExtender> ||
      std::is_same_v<T, UnmanagedMods>;
};
